...
local Races = {}
RegisterServerEvent('ND_races:NewRace')
AddEventHandler('ND_races:NewRace', function(RaceTable)
    local src = source
    ...
    if xPlayer.removeMoney(Races[RaceId].amount) then
        TriggerClientEvent('ND_races:SetRace', -1, Races)
        TriggerClientEvent('ND_races:SetRaceId', src, RaceId)
        TriggerClientEvent('esx:showNotification', src, "You've joined a race for ~g~$"..Races[RaceId].amount)
    end
end)

RegisterServerEvent('ND_races:RaceWon')
AddEventHandler('ND_races:RaceWon', function(RaceId)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ...
    TriggerClientEvent('ND_races:SetRace', -1, Races)
    TriggerClientEvent('ND_races:RaceDone', -1, RaceId, GetPlayerName(src))
end)

RegisterServerEvent('ND_races:JoinRace')
AddEventHandler('ND_races:JoinRace', function(RaceId)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
TriggerEvent('es:addCommand', 'race', function(source, args, user)
    ...
    if GetJoinedRace(GetPlayerIdentifiers(src)[1]) == 0 then
        if xPlayer.getMoney() >= amount then
            TriggerClientEvent('ND_races:CreateRace', src, amount)
        else
...
TriggerEvent('es:addCommand', 'startrace', function(source, args, user)
    ...
    if RaceId ~= 0 then
        ...
        TriggerClientEvent('ND_races:SetRace', -1, Races)
        TriggerClientEvent("ND_races:StartRace", -1, RaceId)
    else
    ...
    for key, race in pairs(Races) do
        if Races[key] ~= nil and Races[key].creator == xPlayer.identifier and not Races[key].started then
            for _, iden in pairs(Races[key].joined) do
                ...
                TriggerClientEvent('ND_races:StopRace', xdPlayer.source)
				RemoveFromRace(iden)
            end
