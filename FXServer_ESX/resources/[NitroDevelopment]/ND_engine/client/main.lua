...
RegisterNetEvent('ND_engine:client:PlayerEnteringVeh')
AddEventHandler('ND_engine:client:PlayerEnteringVeh', function(veh)
...
RegisterNetEvent('ND_engine:client:StartEngineListen')
AddEventHandler('ND_engine:client:StartEngineListen', function()
    Citizen.CreateThread(function()
                            ...
                            if GetVehiclePedIsIn(ped) == vehicle[1] then
                                ...
                                if not vehicle[2] and not vehicle[3] then
                                    ...
                                    if canAttemptHotwire and canSearchForKey then
                                        exports['ND_base']:Print3DText(offCoords, '~y~[Z] ~s~Attempt Hot Wire ~r~| ~y~[G] ~s~Search For Key')
                                    elseif canAttemptHotwire and not canSearchForKey then
                                        exports['ND_base']:Print3DText(offCoords, '~y~[G] ~s~Attempt Hot Wire')
                                    elseif canSearchForKey and not canAttemptHotwire then
                                        exports['ND_base']:Print3DText(offCoords, '~y~[G] ~s~Search For Key')
                                    end
                                elseif not vehicle[2] and vehicle[3] and vehicle[4] then
                                    exports['ND_base']:Print3DText(offCoords, '~y~[F9] ~s~Turn On Engine')
                                elseif not vehicle[4] then
                                    exports['ND_base']:Print3DText(offCoords, 'Vehicle Out Of Fuel')
                                end
                            end
                        end
                    end
...
RegisterNetEvent('ND_engine:client:ForceHotWired')
AddEventHandler('ND_engine:client:ForceHotWired', function()
    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
        Hotwire(GetVehiclePedIsIn(GetPlayerPed(-1)))
    else
        exports['ND_notify']:DoHudText('error', 'Not In Vehicle')
    end
end)
            ...
            if vehicles[StateIndex][3] then
                if vehicles[StateIndex][4] then
                    ...
                    if vehicles[StateIndex][2] then
                        exports['ND_notify']:DoShortHudText('inform', 'Engine Turned On')
                    else
                        exports['ND_notify']:DoShortHudText('inform', 'Engine Turned Off')
                    end
                else
                    exports['ND_notify']:DoHudText('error', 'Vehicle Is Out Of Fuel')
                end
            else
                exports['ND_notify']:DoLongHudText('error', 'Unable to interact with vehicle\'s engine')
            end
		end
    end
                            ...
                            exports['ND_progbar']:Progress({
                                ...
                                if not status then
                                    ...
                                    if successRoll <= success then
                                        Hotwire(veh)
                                        exports['ND_notify']:DoHudText('success', 'Vehicle Hot Wired')
                                    else
                                        exports['ND_notify']:DoHudText('error', 'Hot Wiring Failed')
                                    end
                                else
                                    canAttemptHotwire = true
                                    exports['ND_notify']:DoHudText('error', 'Hot Wiring Cancelled')
                                end
                            end)
                        else
                            isAttemptingHotwire = false
    ...
    exports['ND_progbar']:Progress({
        ...
        if not status then
            ...
            if firstChance > 20 then
                isSearching = false
                Hotwire(veh)
                exports['ND_notify']:DoHudText('success', 'You found a key')
            else
                exports['ND_progbar']:Progress({
                    name = "lockpick_action",
                    duration = 35000,
                    ...
                    if not status then
                        ...
                        if successRoll <= success then
                            ...
                            exports['ND_notify']:DoHudText('error', 'You didn\'t find anything')
                        end
                    else
                        canSearchForKey = true
                        exports['ND_notify']:DoHudText('error', 'Search Cancelled')
                    end
                end)
                isSearching = false
            end
