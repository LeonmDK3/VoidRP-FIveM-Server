...
Citizen.CreateThread(function()
    while true do
      ...
      if (IsControlJustPressed(1, Config.openKey)) then
            ...
            if isInVehicle() and playerData.job.name == "police,sheriff,statepolice" then
...
Citizen.CreateThread(function()
    while isLoading == true do
        ...
        if ESX.IsPlayerLoaded(PlayerId) and playerData.job.name == "police" then
            ...
            TriggerServerEvent("ND__police-tracker:getActivePlates")
            isLoading = false
        end
    end
...
RegisterNetEvent("ND__police-tracker:updateTimer")
AddEventHandler("ND__police-tracker:updateTimer", function(plate)
    time_out[plate] = time_out[nil]
end)

RegisterNetEvent("ND__police-tracker:updateActivePlate")
AddEventHandler("ND__police-tracker:updateActivePlate", function(plate)
...
RegisterNetEvent("ND__police-tracker:getActivePlates")
AddEventHandler("ND__police-tracker:getActivePlates", function(plates)
    time_out = plates
    for v,k in pairs(time_out) do
        checkVehicle(v)
    end
...
RegisterNetEvent('ND__police-tracker:plate')
AddEventHandler('ND__police-tracker:plate', function(plate)
    checkVehicle(plate)
end)
    ...
    for i=1, #vehicle, 1 do
        ...
        if vehicleProps.plate == data.plate then
            ...
            if nCheck >= Config.maxTracker then
                ...
                SetNuiFocus(false)
                TriggerServerEvent("ND__police-tracker", data.plate)
            end
        else
            miss = miss + 1
        end
...
RegisterNUICallback("removeSearch", function(data, cb)
    ...
    for i=1, #vehicle, 1 do
        ...
        if vehicleProps.plate == data.plate then
            TriggerServerEvent("ND__police-tracker:removeActivePlate", data.plate)
            SendNUIMessage({
                type = "ui",
                display = false
              })
...
function checkVehicle(plate)
    ...
    for i=1, #vehicle, 1 do
        ...
        if vehicleProps.plate == plate then
            TriggerServerEvent("ND__police-tracker:setActivePlates", plate)
...
function triggerTimer(plate)
    TriggerServerEvent("ND__police-tracker:triggerTimer", plate)
end
        ...
        Citizen.CreateThread(function()
            while time_out[plate] == false do
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    time_out[plate] = time_out[nil]
                    TriggerServerEvent("ND__police-tracker:removeActivePlate", plate)
