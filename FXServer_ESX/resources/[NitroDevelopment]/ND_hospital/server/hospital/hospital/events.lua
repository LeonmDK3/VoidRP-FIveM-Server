...
RegisterServerEvent('ND_hospital:server:RequestBed')
AddEventHandler('ND_hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('ND_hospital:client:SendToBed', source, k, v)
            return
        end
    end

    TriggerClientEvent('ND_notify:client:SendAlert', source, { type = 'error', text = 'No Beds Available' })
end)

RegisterServerEvent('ND_hospital:server:RPRequestBed')
AddEventHandler('ND_hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('ND_hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('ND_hospital:server:EnteredBed')
AddEventHandler('ND_hospital:server:EnteredBed', function()
    local src = source
    ...
    if BillPlayer(src, totalBill) then
        ...
        TriggerClientEvent('ND_hospital:client:FinishServices', src, false, true)
    else
        ...
        TriggerClientEvent('ND_hospital:client:FinishServices', src, false, false)
    end
end)

RegisterServerEvent('ND_hospital:server:LeaveBed')
AddEventHandler('ND_hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)
