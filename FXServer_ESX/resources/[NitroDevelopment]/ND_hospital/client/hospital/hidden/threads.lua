                        ...
                        if IsControlJustReleased(0, Config.Keys.Revive) then
                            if not usedHiddenRev then
                                if (GetEntityHealth(PlayerPedId()) < 200) or (IsInjuredOrBleeding()) then
                                    exports['ND_progbar']:ProgressWithStartEvent({
                                        ...
                                        if not status then
                                            ...
                                            TriggerServerEvent('ND_hospital:server:AttemptHiddenRevive')
                                        end
                                    end)
                                else
                                    exports['ND_notify']:SendAlert('error', Config.Strings.NotHurt)
                                end
                            else
                                exports['ND_notify']:SendAlert('error', Config.Strings.HiddenCooldown)
                            end
                        end
                    end
                end
