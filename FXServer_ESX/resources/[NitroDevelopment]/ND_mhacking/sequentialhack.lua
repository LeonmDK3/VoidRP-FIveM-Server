...
AddEventHandler('ND_mhacking:seqstart', function(solutionlength, duration, callback)
	if type(solutionlength) ~= 'table' and type(duration) ~= 'table' then
		TriggerEvent('ND_mhacking:show')
		TriggerEvent('ND_mhacking:start', solutionlength, duration, ND_mhackingSeqCallback)
		while seqSwitch == nil do
			Citizen.Wait(5)
		end
		TriggerEvent('ND_mhacking:hide')
	...
	elseif type(solutionlength) == 'table' and type(duration) ~= 'table' then
		...
		for _, sollen in pairs(solutionlength) do
			TriggerEvent('ND_mhacking:start', sollen, seqRemaingingTime, ND_mhackingSeqCallback)
			while seqSwitch == nil do
				Citizen.Wait(5)
			end
	...
	elseif type(solutionlength) ~= 'table' and type(duration) == 'table' then
		...
		for _, dur in pairs(duration) do
			TriggerEvent('ND_mhacking:start', solutionlength, dur, ND_mhackingSeqCallback)
			while seqSwitch == nil do
				Citizen.Wait(5)
			end
	...
	elseif type(solutionlength) == 'table' and type(duration) == 'table' then
		...
		for idx in ipairs(itrTbl) do
			TriggerEvent('ND_mhacking:start', solutionlength[idx], duration[idx], ND_mhackingSeqCallback)
			while seqSwitch == nil do
				Citizen.Wait(5)
			end
...
function ND_mhackingSeqCallback(success, remainingtime)
	seqSwitch = success
