...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('ND_doorlock:getDoorInfo', function(doorInfo, count)
...
RegisterNetEvent('ND_doorlock:currentlyhacking')
AddEventHandler('ND_doorlock:currentlyhacking', function(mycb)
			mycb = true
...
if success then
	...
	TriggerServerEvent('ND_doorlock:updateState', i, doorID.locked)
	...
	print('Failure')
		TriggerEvent('mhacking:hide')
				...
				if IsControlJustReleased(0, Keys['H']) then
					TriggerServerEvent('ND_doorlock:hack', mycb)

					TriggerServerEvent('ND_phone:send', "police", "Someone is attempting a jailbreak!", true)
				end
				...
				if IsControlJustReleased(0, Keys['E']) then
					if isAuthorized then
						...
						TriggerServerEvent('ND_doorlock:updateState', i, doorID.locked) -- Broadcast new state of the door to everyone
					end
				end
			end
		end
...
RegisterNetEvent('ND_doorlock:setState')
AddEventHandler('ND_doorlock:setState', function(doorID, state)
