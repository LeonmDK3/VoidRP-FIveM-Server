...
local isND_skincreatorOpened = true		-- Change this value to show/hide UI
...
function ND_skincreator(enable)
	local ped = GetPlayerPed(-1)
	ShowND_skincreator(enable)
...
function ShowND_skincreator(enable)
	SetNuiFocus(enable)
	SendNUIMessage({
		openND_skincreator = enable
	})
end
...
Citizen.CreateThread(function()
	while true do
		if isND_skincreatorOpened == true then
			ND_skincreator(true)
			SetEntityHeading(GetPlayerPed(-1), heading)
		else
			ND_skincreator(false)
		end
		Citizen.Wait(0)
	end
end)
