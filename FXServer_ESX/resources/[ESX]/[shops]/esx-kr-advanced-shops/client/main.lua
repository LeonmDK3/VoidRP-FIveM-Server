...
AddEventHandler('esx_kr_shop:hasEnteredMarker', function(zone)
	if zone == 'center' then
		...
		exports['mythic_notify']:SendAlert('inform, 'Press To Open Center', 2500, { ['background-color'] = '#2F5C73', ['color'] = '#000000' })
	elseif zone <= 100 then
		ShopId = zone
		number = zone
		Msg  = _U('press_to_open')
		exports['mythic_notify']:SendAlert('inform, 'Press To Open', 2500, { ['background-color'] = '#2F5C73', ['color'] = '#000000' })
	elseif zone >= 100 then
		ShopId = zone
		number = zone
		Msg  = _U('press_to_rob')
		exports['mythic_notify']:SendAlert('inform, 'Press To Rob', 2500, { ['background-color'] = '#2F5C73', ['color'] = '#000000' })
	end
end)
  ...
  function(data, menu)
        ...
        local invcount = data.current.count
			...
			if count > invcount then
				...
				exports['mythic_notify']:SendAlert('inform, 'You Can't sell more than you own', 2500, { ['background-color'] = '#2F5C73', ['color'] = '#000000' })
				menu2.close()
				menu.close()
			else
				menu2.close()
		...
		ESX.TriggerServerCallback('esx_kr_shop-robbery:getOnlinePolices', function(results)
			...
			if result.cb ~= nil then
                ...
                TriggerServerEvent('esx_kr_shops-robbery:UpdateCanRob', id)
					...
					exports['mythic_notify']:SendAlert('error, 'Not Enough Police On Duty', 2500, { ['background-color'] = '#2F5C73', ['color'] = '#000000' })
				end
			else
				...
				exports['mythic_notify']:SendAlert('error, 'Recently Robbed!', 2500, { ['background-color'] = '#E03232', ['color'] = '#000000' })
			end
		end)
	end, id)
end
        ...
        Wait(0)
			local playerpos = GetEntityCoords(GetPlayerPed(-1))
				...
				elseif OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) >= 15 then
					...
					exports['mythic_notify']:SendAlert('error, 'You Have Canceled the Robbery', 2500, { ['background-color'] = '#E03232', ['color'] = '#000000' })
				end
	end
end)
