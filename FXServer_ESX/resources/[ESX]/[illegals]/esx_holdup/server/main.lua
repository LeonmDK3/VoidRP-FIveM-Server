	...
	if Stores[currentStore] then
		...
		if not rob then
			if cops >= Config.PoliceNumberRequired then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Config.PoliceNumberRequired', style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
			end
		else
			TriggerClientEvent('esx:showNotification', _source, _U('robbery_already'))
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Robbery Already', style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
		end
	end
end)
